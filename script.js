  ...
  products.forEach((product) => {
    ...
    productElement.innerHTML = `
            <picture>
                ...
                <source media="(min-width: 604px)" srcset="${
                  product.image.tablet
                }">
                <img class="product__img" src="${product.image.mobile}" alt="${
      product.name
    }" title="${product.name}">
            </picture>
...
bodyElement.addEventListener("click", (e) => {
  ...
  function updateCartTitle() {
    ...
    cartTitle.innerText = `Your Cart (${Cart.reduce(
  ...
  if (productElement) {
    ...
    function updateProductInfo() {
      const cartProduct = document.querySelector(
        `.cart-product[data-index="${productData.index}"]`
      );
      cartProduct.innerHTML = `
                <div>
                    <p class="cart-product-name">${
                      Cart[cartProduct.dataset.index].name
                    }</p>
                    <span class="cart-product-quantity">${
                      Cart[cartProduct.dataset.index].quantity
                    }x</span>
                    <span class="cart-product-price">@ $${
                      Cart[cartProduct.dataset.index].price
                    }</span>
                    <span class="cart-product-total-price">$${
                      Cart[cartProduct.dataset.index].totalPrice
                    }</span>
                </div>
    ...
    if (e.target.closest(".add-to-cart-btn")) {
      ...
      displayOrderTotal();
    } else if (decrementQuantityBtn || incrementQuantityBtn) {
      function updateCartTitle() {
        const cartTitle = document.querySelector("#cartTitle");
        cartTitle.innerText = `Your Cart (${Cart.reduce(
          (acc, product) => acc + product.quantity,
      ...
      if (Cart.length > 0 && productData.index) {
        Cart[productData.index].totalPrice =
          productData.price * Cart[productData.index].quantity;
        Cart[productData.index].totalPrice =
          Cart[productData.index].totalPrice.toFixed(2);
        updateProductQuantity();
        updateProductInfo();
      }
    ...
    const productElement = productList.querySelector(
      `.product[data-index="${cartProduct.dataset.index}"]`
    );
